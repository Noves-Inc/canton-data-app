...
> **v4 pre-release branch:** The v4 database image is published, but the public v4 backend and
> frontend images have not yet been published. The legacy `canton-translate` image references
> still present in this branch are v3 artifacts and must not be used for a v4 deployment or
> upgrade. This warning will be removed only after release-pinned public v4 images are available
> and the Compose and Kubernetes procedures pass their clean-install and v3.15-upgrade gates.
...
Major-version upgrades from Python v3 are covered by
[Upgrade from Data App v3.15.0 to v4](upgrade_v3_15_to_v4.md).
...
- **Database**: `ghcr.io/noves-inc/cda-db:v4.0.0`
...
| **Validator Uptime** | | **Local participant readiness and public on-ledger liveness monitoring. Enabled by default.** |
| `VALIDATOR_UPTIME_ENABLED` | `true` | Master gate. Set to `false` to suppress the uptime workers, public read API, and UI feature. |
| `VALIDATOR_UPTIME_INTERVAL_SECONDS` | `30` | Probe interval, from 15 through 300 seconds. |
| `VALIDATOR_UPTIME_TIMEOUT_SECONDS` | `3` | Per-probe participant deadline, from 1 through 10 seconds. |
| `VALIDATOR_UPTIME_RAW_RETENTION_DAYS` | `35` | Raw sample retention, from 7 through 90 days. |
| `VALIDATOR_UPTIME_HOURLY_RETENTION_DAYS` | `400` | Hourly rollup retention; must be at least the raw retention and no more than 1,825 days. |
| `PUBLIC_SCAN_INDEXER_URL` | `https://indexer.example` | Public validator index used for on-ledger liveness and automatic validator-party discovery. |

**Note:** User-scoped requests pass the frontend JWT through to Canton's Ledger API. The v4
background capture worker separately uses a confidential client-credentials identity and must
receive its client secret through a secret store, not a ConfigMap or checked-in environment file.
